...
function getReleaseVersion(): string {
  ...
  try {
    ...
    if (typeof version === 'string') {
      ...
      console.error(`Error: Could not find a valid version string in ${manifestPath}`);
      process.exit(1);
    }
  } catch (error) {
...
function main() {
  ...
  switch (versionArg) {
    ...
    default:
      console.error(`Error: Invalid version type '${versionArg}'. Please use 'main' or 'release'.`);
      console.error(`Usage: ts-node ${path.basename(__filename)} [main|release]`);
      process.exit(1); 
  }
}
